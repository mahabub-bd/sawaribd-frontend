...
const AdminDashboard = async () => {
  ...
  try {
    const responseData = await fetchProtectedData(`user-activity/${userId}/recent`)
    ...
    activityError = error instanceof Error ? error.message : "Failed to load activity data"
  }
  ...
  const getActionInfo = (action: string) => {
    ...
    } else if (actionLower.includes("sign out") || actionLower.includes("logout")) {
      return {
  ...
  const getActivityDescription = (path: string, action: string) => {
    ...
    if (pathParts[0] === "auth") {
      if (action.toLowerCase().includes("signin") || action.toLowerCase().includes("sign in")) {
        return "User signed in to the system"
      } else if (action.toLowerCase().includes("signout") || action.toLowerCase().includes("sign out")) {
        ...
        <Card className="w-full shadow-md hover:shadow-lg transition-shadow duration-300">
          ...
          <CardContent className="pt-4">
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-muted-foreground flex-1 truncate">{userId}</span>
          ...
          <CardFooter className="pt-4">
            <div className="w-full flex justify-between items-center">
              <span className="text-sm text-muted-foreground">Last login: Today at 10:23 AM</span>
        ...
        <Card className="w-full shadow-md hover:shadow-lg transition-shadow duration-300">
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              {userActivity.length > 0 && <Badge variant="secondary">{userActivity.length}</Badge>}
            </div>
          </CardHeader>
          <CardContent>
            {activityError ? (
              ...
              <div className="space-y-4 max-h-[350px] overflow-y-auto pr-1">
                {userActivity.map((activity) => {
                  ...
                  const { color, icon } = getActionInfo(activity.action || "Unknown")
                  return (
                    <div
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <span className="text-xs text-muted-foreground flex items-center gap-1">
                            ...
                            {activity.createdAt ? formatActivityDate(activity.createdAt) : "Unknown time"}
                          </span>
                        </div>
                        <p className="text-sm line-clamp-2">
                          {activity.path
                            ? getActivityDescription(activity.path, activity.action || "Unknown")
                            : "Unknown activity"}
                        </p>
                        {activity.path && <p className="text-xs text-muted-foreground mt-1">Path: {activity.path}</p>}
                      </div>
                    </div>
                  )
                })}
          ...
          <CardContent>
            <div className="grid grid-cols-2 gap-4">
              <button className="bg-primary/10 hover:bg-primary/20 transition-colors rounded-lg p-4 text-left">
                ...
                <p className="text-sm text-muted-foreground">Review account security settings</p>
              </button>
              <button className="bg-primary/10 hover:bg-primary/20 transition-colors rounded-lg p-4 text-left">
                <div className="flex items-center gap-2 mb-2">
